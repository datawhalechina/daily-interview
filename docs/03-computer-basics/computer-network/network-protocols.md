...
1. 浏览器解析URL，查�?DNS，检查域名是否在缓存中（浏览器自身的DNS缓存、操作系统的DNS缓存、本地的hosts文件和向本地DNS服务器进行查询）。如�?DNS 服务器和主机在同一个子网内，系统会按照 ARP 过程�?DNS 服务器进�?ARP查询，如果在不同的子网，系统会按�?ARP 过程对默认网关进行查询；
...
[超详细版�?](https://github.com/skyline75489/what-happens-when-zh_CN)

[超详细版�?](https://segmentfault.com/a/1190000006879700)
...
> * 四次握手：TCP 协议的设计可以让我们同时传�?`ACK` �?`SYN` 两个控制信息，减少了通信次数，所以不需要使用更多的通信次数传输相同的信息�?
...
* 无连接的协议，提供无连接服务�?
* 面向字节流，其传送的运输协议数据单元TPDU�?UDP报文或用户数据报�?
* 支持单播、多播、广播；
* 不可靠传输，不使用流量控制和拥塞控制�?
* 简单，适用于很多应用。如：多媒体应用等�?
...
* **超时重传�?*�?TCP 发出一个段后，它启动一个定时器，等待目的端确认收到这个报文段。如果不能及时收到一个确认，将重发这个报文段�?
...
* 慢开�?
  ...
  * 刚开始发送数据时，先把拥塞窗口cwnd（congestion window）设置为一个最大报文段MSS的数值，每收到一个新的确认报文之后，可以把拥塞窗口增加最多一�?SMSS 的数值。这样每经过一个传输轮次（或者说是每经过一个往返时间RTT），拥塞窗口的大小就会加倍�?

    ![](https://i.loli.net/2021/03/12/lpe2o7CVsbJaNj1.png)

* 拥塞避免

  * 当拥塞窗口的大小达到慢开始门�?slow start threshold)时，开始执行拥塞避免算法，让拥塞窗口cwnd缓慢地增大，避免出现拥塞，每经过一个传输轮次，拥塞窗口加一，使其按线性规律缓慢增大�?
...
�?	Session 代表着服务器和客户端一次会话的过程。Session 对象存储特定用户会话所需的属性及配置信息。这样，当用户在应用程序�?Web 页之间跳转时，存储在 Session 对象中的变量将不会丢失，而是在整个用户会话中一直存在下去。当客户端关闭会话，或�?Session 超时失效时会话结束�?

**区别**�?

* 作用范围不同，Cookie 保存在客户端（浏览器），Session 保存在服务器端；
* 存取方式不同，Cookie 只能保存 ASCII，Session 可以存任意数据类型；
* 有效期不同，Cookie 可设置为长时间保持，Session 一般失效时间较短，客户端关闭或�?Session 超时都会失效�?
* 隐私策略不同，Cookie 存储在客户端，比较容易遭到不法获取，Session 存储在服务端，安全性相�?Cookie 要好一些；
* 存储大小不同�?单个 Cookie 保存的数据不能超�?4K，Session 可存储数据远高于 Cookie�?

### 11.HTTP状态码

* 2xx状态码：操作成功�?00 OK�?

* 3xx状态码：重定向�?01 永久重定向；302暂时重定向；
* 4xx状态码：客户端错误�?00 Bad Request�?01 Unauthorized�?03 Forbidden�?04 Not Found�?
* 5xx状态码：服务端错误�?00服务器内部错误；501服务不可用；

### 12.HTTP报文

HTTP报文有请求报文和响应报文两种，请求报文：从客户向服务器发送请求报文；响应报文：从服务端到客户的回答。HTTP报文都由**开始行、首部行、实体主�?*三部分组成�?

* **开始行**
  * 请求报文
    * �?**方法**�?*[空格]**�?*URL**�?*[空格]**�?*HTTP版本** 组成�?
  * 响应报文
    * �?**HTTP 版本**�?*[空格]**�?*状态码**组成�?
* **首部�?*
...
* 当主机A欲向本局域网上的某个主机 B 发�?IP 数据报时，就先在�?ARP高速缓存中查看有无主机B的IP地址�?
...
* IPv4协议的数据包需�?76个字节，碎片可�?；IPv6协议的数据包需�?280个字节，不会碎片�?
* IPv4根据提供�?IP 选项，有 20-60 个字节的可变长度；IPv6�?0 个字节的固定长度。没�?IP 报头选项。通常，IPv6 报头�?IPv4 报头简单�?

### 4.对称加密和非对称加密在HTTPS的应�?

**对称加密算法**：发送方和接收方需要持有同一把密钥，发送消息和接收消息均使用该密钥。相对于非对称加密，对称加密具有更高的加解密速度，但双方都需要事先知道密钥，密钥在传输过程中可能会被窃取，因此安全性没有非对称加密高�?

**非对称加密算�?*：接收方在发送消息前需要事先生成公钥和私钥，然后将公钥发送给发送方。发送放收到公钥后，将待发送数据用公钥加密，发送给接收方。接收到收到数据后，用私钥解密。在这个过程中，公钥负责加密，私钥负责解密，数据在传输过程中即使被截获，攻击者由于没有私钥，因此也无法破解。非对称加密算法的加解密速度低于对称加密算法，但是安全性更高�?

HTTPS中同时使用了**对称加密算法**�?*非对称加密算�?*。对于一个完整的HTTPS请求过程�?
...
* [字节跳动教育prek客户端日常实�?一二三面面经](https://www.nowcoder.com/discuss/653809?type=2&channel=1009&source_id=discuss_center_discuss_hot_nctrack)
