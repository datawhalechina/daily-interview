...
模型的预测误差可以分解为三个部分: 偏差(bias)�?方差(variance) 和噪�?noise).
...
- 噪声表达了在当前任务上任何模型所能达到的期望泛化误差的下界， 即刻画了学习问题本身的难�?�?

  下图�?*偏差和方差示意图**�?

![image-20210927150625116](/images/过拟合与欠拟�?image-20210927150625116.png)

�?                                                                                              

**泛化误差、偏差、方差和模型复杂度的关系**（图片来源百面机器学习）

<img src="/images/过拟合与欠拟�?JrBui5yzA3IGgk7.png" alt="image-20210423092851506"  />
...
![img](/images/过拟合与欠拟�?247f6539-1c10-75ac-84f8-02d238699dfd.jpg)
...
- 调整模型的容�?capacity)，通俗地，模型的容量是指其拟合各种函数的能力；
- 容量低的模型可能很难拟合训练集�?

## 4. 怎么解决过拟合（重点�?

- 获取和使用更多的数据（数据集增强）——解决过拟合的根本性方�?
...
![img](/images/过拟合与欠拟�?v2-a026e24156e13a1d14c43df26b9bd2a4_720w.jpg)

![img](/images/过拟合与欠拟�?v2-f6edae58134c5a26687c3883af48d5d5_720w.jpg)

![img](/images/过拟合与欠拟�?v2-3aaa69f70754c469bca5c8e4c3e161db_720w.jpg)
...
* 取平均的作用
  ...
  先回到标准的模型即没有dropout，我们用相同的训练数据去训练5个不同的神经网络，一般会得到5个不同的结果，此时我们可以采�?�?个结果取均值”或者“多数取胜的投票策略”去决定最终结果。例�?个网络判断结果为数字9，那么很有可能真正的结果就是数字9，其它两个网络给出了错误结果。这种“综合起来取平均”的策略通常可以有效防止过拟合问题。因为不同的网络可能产生不同的过拟合，取平均则有可能让一些“相反的”拟合互相抵消。dropout掉不同的隐藏神经元就类似在训练不同的网络，随机删掉一半隐藏神经元导致网络结构已经不同，整个dropout过程就相当于对很多个不同的神经网络取平均。而不同的网络产生不同的过拟合，一些互为“反向”的拟合相互抵消就可以达到整体上减少过拟合�?

* 减少神经元之间复杂的共适应关系

  因为dropout程序导致两个神经元不一定每次都在一个dropout网络中出现。这样权值的更新不再依赖于有固定关系的隐含节点的共同作用，阻止了某些特征仅仅在其它特定特征下才有效果的情�?。迫使网络去学习更加鲁棒的特�?，这些特征在其它的神经元的随机子集中也存在。换句话说假如我们的神经网络是在做出某种预测，它不应该对一些特定的线索片段太过敏感，即使丢失特定的线索，它也应该可以从众多其它线索中学习一些共同的特征。从这个角度看dropout就有点像L1，L2正则，减少权重使得网络对丢失特定神经元连接的鲁棒性提高�?

* Dropout类似于性别在生物进化中的角�?
...
![img](/images/过拟合与欠拟�?96b02ee9-c6ab-718f-b358-0a8db26f9cd7.jpg)
...
![img](/images/过拟合与欠拟�?f4a1eb7a-3ce3-7cea-84a0-d569423b2d01.jpg)
