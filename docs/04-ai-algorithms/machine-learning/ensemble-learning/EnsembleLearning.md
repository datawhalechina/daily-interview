...
**集成学习算法是一种优化手段或者策�?*，将多个较弱的模型集成模型组，一般的弱分类器可以是决策树，SVM，KNN等构成。其中的模型可以单独进行训练，并且它们的预测能以某种方式结合起来去做出一个总体预测�?
...
* 基于数据**随机重抽�?*的分类器构建方法。从训练集中进行子抽样组成每个基模型所需要的子训练集，对所有基模型预测的结果进行综合产生最终的预测结果�?
...
* 对于$t=1�? \dots T$
  * 对训练集进行第t次随机采样，共采�?T$次，得到包含$T$个样本的采样�?D_t$
  * 用采样集$D_t$训练�?t$个弱学习�?G_t(x)$
...
* 将训练好的所有基模型对训练集进行预测，第�?i$基模型对�?i$个训练样本的预测值将作为新的训练集中�?i$个样本的�?i$个特征值，最后基于新的训练集进行训练。同理，预测的过程也要先经过所有基模型的预测形成新的测试集，最后再对测试集进行预测�?
...
* RF有两种方法：
  ...
  - 对于一颗树，先使用**袋外错误�?*(OOB)样本计算测试误差a，再随机打乱OOB样本中第i个特征（上下打乱特征矩阵第i列的顺序）后计算测试误差b，a与b差距越大特征i越重要�?

- GBDT计算方法�?
  - 所有回归树中通过特征i分裂后平方损失的减少值的�?回归树数�?得到特征重要性�?在sklearn中，GBDT和RF的特征重要性计算方法是相同的，都是基于单棵树计算每个特征的重要性，探究每个特征在每棵树上做了多少的贡献，再取个平均值�?
