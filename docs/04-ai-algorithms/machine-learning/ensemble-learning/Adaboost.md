![Adboost框架图](/images/Adaboost/Adboost框架�?png)
...
一类是以boosting、Adaboost等算法为代表的，**个体学习器是串行序列化生成的、具有依赖关�?*，它试图不断增强单个学习器的学习能力�?
...
�?	其中 $Z_t$是规范化因子，使�?D(t+1)$成为一个概率分布（和为1）：
$$
...
## 3. 算法面试�?
### 3.1 Adaboost分类模型的学习器的权重系�?\alpha$怎么计算的？
...
2. 目标是使前向分步算法得到�?\alpha$�?G_m(x)$�?f_m(x)$在训练数据集T上的指数损失函数最小，�?
   $$
   (\alpha, G_m(x))=arg min_{\alpha, G}\sum_{i=1}^{N}exp[-y_i(f_{m-1}(x_i)+\alpha G(x_i))]
   $$
   其中�?\hat{w}_{mi}=exp[-y_i f_{m-1}(x_i)].$为了求上式的最小化，首先计�?G_m^*(x)$,对于任意�?\alpha >0$,可以转化为下式：
   $$
   G_{m}^*=argmin_{G}\sum_{i=1}^{N}\hat{w}_{mi}I(y_i \neq G(x_i))
   $$
   之后�?\alpha_m^*$,将上述式子化简，得�?
...
将已经求得的$G_m^*(x)$带入上式面，�?\alpha$求导并等�?，得到最优的$\alpha$.
$$
...
**�?** **不管是分类问题还是回归问题，根据误差改变权重就是Adaboost的本质，可以基于这个构建相应的强学习器�?*

### 3.3 boosting和bagging之间的区�?从偏�?方差的角度解释Adaboost�?
...
每轮训练结束后，AdaBoost 会对样本的权重进行调整，调整的结果是越到后面被错误分类的样本权重会越高。而后面的分类器为了达到较低的带权分类误差，会把样本权重高的样本分类正确。这样造成的结果是，虽然每个弱分类器可能都有分错的样本，然而整�?AdaBoost 却能保证对每个样本进行正确分类，从而实现快速收敛�?
...
1. 台湾清华大学李端兴教�?017年秋机器学习概论课程(CS 4602)PPT
2. 周志�?《机器学习》第8�?集成学习
