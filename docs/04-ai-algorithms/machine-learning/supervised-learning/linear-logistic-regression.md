...
**线性回�?Linear Regression)是利用称为线性回归方程的最小平方函数对一个或多个自变量和因变量之间关系进行建模的一种回归分析�?*这种函数是一个或多个称为回归系数的模型参数的线性组合。只有一个自变量的情况称为简单回�?大于一个自变量情况的叫做多元回归�?

- 线性：两个变量之间的关�?*�?*一次函数关系的——图�?*是直�?*，叫做线性�?
- 非线性：两个变量之间的关�?*不是**一次函数关系的——图�?*不是直线**，叫做非线性�?
- 回归：人们在测量事物的时候因为客观条件所限，求得的都是测量值，而不是事物真实的值，为了能够得到真实值，无限次的进行测量，最后通过这些测量数据计算**回归到真实�?*，这就是回归的由来�?

线性回归就是利用的样本$D=(\mathrm{x}_i, \mathrm{y}_i)
$�? \mathrm{i}=1,2,3 \ldots \mathrm{N}, \mathrm{x}_i$是特征数据，可能是一个，也可能是多个，通过有监督的学习，学习到�?x$�?y$的映�?h$，利用该映射关系对未知的数据进行预估，因�?y$为连续值，所以是回归问题�?
...
## 3. 线性回归的损失函数是什么形式？
一般使�?*最小二乘法**，真实�?y_{i}$，预测�?h_θ(x)$，则误差平方�?\left(y_{i}-h_{\theta}\left(x_{i}\right)\right)^{2}$找到合适的参数，使得误差平方和最�?
$$
MSE: J\left(\theta_{0}, \theta_{1}\right)=\frac{1}{2 m} \sum_{i=1}^{m}\left(y_{i}-h_{\theta}\left(x_{i}\right)\right)^{2}
$$

其中共有$m$个样本点，乘�?/2是为了方便计算�?
...
* 目的�?
  ...
  * 解决在通过正规方程方法求解$\theta$的过程中出现�?X^TX$不可逆的请况�?
...
* 补充
  ...
  * **ElasticNet 回归**�?线性回�?+ L1正则�?+ L2 正则化�?

    * ElasticNet在我们发现用Lasso回归太过(太多特征被稀疏为0),而岭回归也正则化的不�?回归系数衰减太慢)的时候，可以考虑使用ElasticNet回归来综合，得到比较好的结果�?
  ...
  * **LWR( 局部加�?回归**�?
    ...
    * 局部加权线性回归是在线性回归的基础上对每一个测试样本（训练的时候就是每一个训练样本）在其已有的样本进行一个加权拟合，**权重的确�?*可以通过一个核来计算，常用的有**高斯�?*（离测试样本越近，权重越大，反之越小），这样对每一个测试样本就得到了不一样的权重向量，所以最后得出的拟合曲线不再是线性的了，这样就增加的模型的复杂度来更好的拟合非线性数据�?
...
逻辑回归主要用来解决**分类**问题，线性回归的结果$Y$带入一个非线性变换的**Sigmoid函数**中，得到$[0,1]$之间取值范围的�?S$�?S$可以把它看成是一个概率值，如果我们设置概率阈值为0.5，那�?S$大于0.5可以看成是正样本，小�?.5看成是负样本，就可以进行分类了�?
...
函数�?t$无论取什么值，其结果都�?[0,{1}]$的区间内，我们假设分类的**阈�?*�?.5，那么超�?.5的归�?分类，低�?.5的归�?分类，阈值是可以自己设定的�?

下来我们把线性回归的输出 $\theta^T X+b$ 带入 $t$ 中就得到了逻辑回归�?*假设函数**�?

$$
H(\theta, b)=\frac{1}{1+e^{(\theta^T X+b)}}
$$
结果也可以理解为概率，换句话说概率大�?.5的属�?分类，概率小�?.5的属�?分类，这就达到了分类的目的�?
...
* 一对一�?
  * �?N$个类别两两配对，产生$N(N-1)/2$个二分类任务，测试阶段新样本同时交给所有的分类器，最终结果通过投票产生�?

* 一对多�?
  * 每一次将一个例作为正例，其他的作为反例，训�?N$个分类器，测试时如果只有一个分类器预测为正类，则对应类别为最终结果，如果有多个，则一般选择置信度最大的。从分类器角度一对一更多，但是每一次都只用�?个类别，因此当类别数很多的时候一对一开销通常更小(只要训练复杂度高�?O(N)$即可得到此结�?�?
...
* 缺点
  ...
  * 预测结果�?S$型，因此�?log(odds)$向概率转化的过程是非线性的，在两端随着$log(odds)$值的变化，概率变化很小，边际值太小，slope太小，而中间概率的变化很大，很敏感�?导致很多区间的变量变化对目标概率的影响没有区分度，无法确定阀值�?
...
* 方便交叉与特征组合：离散化后可以进行特征交叉，由$M+N$个变量变�?M*N$个变量，进一步引入非线性，提升表达能力�?

* 简化模型：特征离散化以后，起到了简化了逻辑回归模型的作用，降低了模型过拟合的风险�?
* 稳定性：特征离散化后，模型会更稳定，比如如果对用户年龄离散化�?0-30作为一个区间，不会因为一个用户年龄长了一岁就变成一个完全不同的人；

* 离散化后的特征对异常数据有很强的鲁棒性：比如一个特征是年龄>30�?，否�?。如果特征没有离散化，一个异常数据“年�?00岁”会给模型造成很大的干扰�?
...
- 线性回归的损失函数�?MSE$,逻辑回归中，采用的是负对数损失函�?
...
- 线性模型的输出都是�?[-�?+∞]$之间的，而Sigmoid能够把它映射�?[0,1]$之间。正好这个是概率的范围�?
...
+ LR能做�?SVM能做，但可能在准确率上有问题，SVM能做的LR有的做不了�?
