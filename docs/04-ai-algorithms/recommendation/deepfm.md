...
DeepFM模型�?017年由哈工大与华为联合提出的模型，是对Wide&Deep模型的改进。与DCN不同的是，DeepFM模型是将Wide部分替换为了FM模型，增强了模型的低阶特征交互的能力。关于低阶特征交互，文章的Introduction中也提到了其重要性，例如�?

1、用户经常在饭点下载送餐APP，故存在一�?阶交互：app种类与时间戳�?

2、青少年喜欢射击游戏和RPG游戏，存在一�?阶交互：app种类、用户性别和年龄；

用户背后的特征交互非常的复杂，低阶和高阶的特征交互都是很重要的，这也证明了Wide&Deep这种模型架构的有效性。DeepFM是一�?*端到端的模型**，强调了包括低阶和高阶的特征交互接下来直接对DeepFM模型架构进行介绍，并与其他之前提到过的模型进行简单的对比�?
...
<img src="http://gzy-gallery.oss-cn-shanghai.aliyuncs.com/work_/images/21.png" style="zoom: 50%;" />

在论文中模型的目标是**共同学习低阶和高阶特征交�?*，应用场景依旧是CTR预估，因此是一个二分类任务�?y=1$表示用户点击物品�?y=0$则表示用户未点击物品�?
...
<img src="http://gzy-gallery.oss-cn-shanghai.aliyuncs.com/work_/images/22.png" style="zoom: 50%;" />



文章中指出每个特征域使用的Embedding维度$k$都是相同的�?

【注】与Wide&Deep不同的是，DeepFM中的**Wide部分与Deep部分共享了输入特�?*，即Embedding向量�?



#### Wide部分---FM

<img src="http://gzy-gallery.oss-cn-shanghai.aliyuncs.com/work_/images/23.png" style="zoom:67%;" />



FM模型[^4]�?010年Rendle提出的一个强大的**非线性分类模�?*，除了特征间的线�?1�?相互作用外，FM还将特征间的(2�?相互作用作为各自特征潜向量的内积进行j建模。通过隐向量的引入使得FM模型更好的去处理数据稀疏行的问题，想具体了解的可以看一下原文。DeepFM模型的Wide部分就直接使用了FM，Embedding向量作为FM的输入�?

$$
y_{F M}=\langle w, x\rangle+\sum_{j_{1}=1}^{d} \sum_{j_{2}=j_{1}+1}^{d}\left\langle V_{i}, V_{j}\right\rangle x_{j_{1}} \cdot x_{j_{2}}
$$
其中$w \in \mathbf{R}^d$�?\langle w, x\rangle$表示1阶特征，$V_i \in \mathbf{R}^k$表示�?i$个隐向量�?k$表示隐向量的维度�?$\displaystyle\sum_{j_{1}=1}^{d} \sum_{j_{2}=j_{1}+1}^{d}\left\langle V_{i}, V_{j}\right\rangle x_{j_{1}} \cdot x_{j_{2}}$$表示2阶特征�?

具体的对�?阶特征，FM论文中有下述计算（采取原文的描述形式），为线性复杂复杂度$O(kn)$�?
$$
...
<img src="http://gzy-gallery.oss-cn-shanghai.aliyuncs.com/work_/images/24.png" style="zoom:67%;" />
