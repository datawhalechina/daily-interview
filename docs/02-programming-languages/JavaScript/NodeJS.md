...
#### Q：NodeJS �?IO 模型特点是什么？与多线程同步 IO 有什么不同？

NodeJS �?IO 模型（更准确的说�?js 的执行环境，也就�?v8）的特点是“单线程异步非阻塞”�?

而与多线程同�?IO，两者各有优劣，应该根据实际应用场景来做取舍�?

在传统的观点里，异步 IO 的好处是 IO 本身并不需要占用太多的资源，缺点在于非线性代码带来的复杂度和难以理解维护，而多线程同步 IO 的缺点在于性能资源的开销和线程管理的问题�?

所以很显然，在相同的机器资源里面，异步 IO 的并发量肯定是要高于多线程同�?IO 的；但是服务器程序本身肯定不只是�?IO 组成，还有逻辑运算的部分，过重的逻辑运算依旧会影响性能。换句话说，密集�?CPU 任务会阻�?js 的执行，导致异步 IO 得不到处理，极大地影响到 node 处理响应的时间�?

总之，node �?IO 模型更适合处理 IO 密集型的任务。多线程同步 IO 更适合处理计算密集型的任务�?
...
相反，如果包含对这个值引用的变量又取得了另外一个值，则这个值的引用次数�?1�?
当这个值的引用次数变成 0 时，就可以将其占用的内存空间回收回来，这样，当垃圾收集器下次再运行时，它就会释放�?些引用次数为零的值所占用的内存�?
但这样会有循环引用的问题�?
...
新生代主要使�?Scavenge 进行管理，将内存平均分为两块，使用空间叫 From，闲置空间叫 To，新对象都先分配�?From 空间中，在空间快要占满时将存活对象复制到 To 空间中，然后清空 From 的内存空间，此时，调�?From 空间�?To 空间，继续进行内存分配，当满足晋升条件时对象会从新生代晋升到老生代�?

对象晋升的条件主要有两个�?
如果一个对象是第二次经历从 From 空间复制�?To 空间，那么这个对象会被移动到老生代中�?
当要�?From 空间复制一个对象到 To 空间时，如果 To 空间已经使用了超�?25%，则这个对象直接晋升到老生代中。（设置 25%这个阈值的原因是当这次 Scavenge 回收完成后，这个 To 空间会变�?From 空间，接下来的内存分配将在这个空间中进行。如果占比过高，会影响后续的内存分配�?

老生代主要采�?Mark-Sweep �?Mark-Compact 算法，一个是标记清除，一个是标记整理。两者不同的地方是，Mark-Sweep 在垃圾回收后会产生碎片内存，�?Mark-Compact 在清除前会进行一步整理，将存活对象向一侧移动，随后清空边界的另一侧内存，这样空闲的内存都是连续的，但是带来的问题就是速度会慢一些。在 V8 中，老生代是 Mark-Sweep �?Mark-Compact 两者共同进行管理的�?
